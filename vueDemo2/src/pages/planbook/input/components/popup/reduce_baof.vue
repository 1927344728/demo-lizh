	...
	<div class="mainPopupDiv" @click="ckClosePopup" v-if="allBenefitData && allBenefitData.length">
		<div class="mainContentDiv" @click.stop="">
			<div class="showAccountDataDIv">
				<header>
					<em>{{curInsData.reduceBaofData.accountPart == 'addAndTake' ? '追加领取' : curInsData.reduceBaofData.accountPart == 'add' ? '追加保费' : '减保领取'}}</em>
					<i class="iconfont icon-close_line" @click="ckClosePopup"></i>
					</header>
				<ul class="bonusTypeUl" v-if="bonusTypeOpts && bonusTypeOpts.opts">
					<li v-for="(b, i) in bonusTypeOpts.opts" :value="b.value" :class="bonusType == b.value? 'active' : ''" @click="getBonusType(b.value)">{{b.description}}</li>
				</ul>
				<div class="reduceBaofDataDIv" v-if="popAccountData && popAccountData.ageOpts">
			...
			<div class="changeAccountDataDIv clearFloat">
				<p>
					<label v-if="curInsData.reduceBaofData.accountPart == 'addAndTake' || curInsData.reduceBaofData.accountPart == 'add'">
						<em :class="accountType == 'add' ? 'active' : ''" @click="getAccountType('add')">追加保费</em>
					</label>
					<label v-if="curInsData.reduceBaofData.accountPart == 'addAndTake' || curInsData.reduceBaofData.accountPart == 'take'">
						<em :class="accountType == 'take' ? 'active' : ''" @click="getAccountType('take')">减保领取</em>
					</label>
				</p>
				...
				<div>
					<table v-for="(account, index) in curInsData.reduceBaofData.addAndTakeData" v-if="account && account.adjustData && account.adjustData.length" :key="JSON.stringify(account)">
...
<script>
	...
	export default {
		name: 'PopupReduceBaof',
		data () {
			let purposeOpts = ['高中教育金', '大学教育金', '婚嫁金', '子女教育金', '养老金', '自己填写']
			let accountType = 'take'

			return {
				curModuleData: this.allInputData.curModuleData,
				curProdData: helperJs.deepCopy({}, this.allInputData.curModuleData.curProdData),
		...
		methods: {
			getDefAddAndTakeData () {
				var self = this
				self.curInsData.reduceBaofData = self.curInsData.reduceBaofData || {
					accountPart: 'take',
					addAndTakeData: [
						{ key: 'take', adjustData: [] }
					]
			...
			getAllBenefitData (scroll, callBack) {  //scroll是否滚动到指定位置; callBack回调函数
				let self = this
				self.curProdData.mulInsOrderList.forEach(function (m) {
					if (m.key == self.curInsData.key) {
						for (let k in self.curInsData) {
							m[k] = self.curInsData[k]
						}
					}
				})
				let curAdjustData = helperJs.filterArr(self.curInsData.reduceBaofData.addAndTakeData, self.accountType).adjustData

				let curInsKey
				let eleInsuranceId = helperJs.filterArr(self.curInsData.eleOrderList, 'insuranceId')
				if (self.curInsData.isGroup && eleInsuranceId && helperJs.filterArr(self.curProdData.mulInsOrderList, eleInsuranceId.value)) {
					curInsKey = helperJs.filterArr(self.curProdData.mulInsOrderList, eleInsuranceId.value).key
				}
				helperJs.vueAxios({  //请求计算
					self,
					url: `${baseApiPath}/planBook/V2/drawInsurance/${self.bonusType || 'low'}`,
					...
					if (res.data.success) {
						...
						self.reduceBeginYear = res.data.data.reduceIndex
						...
						curAdjustData.length && curAdjustData.sort(function (a, b) {
							return a[0] - b[0]
						})


						self.adjustData.beginYear = 0
						self.adjustData.endYear = 0
			...
			getYearOpts () {
				...
				helperJs.filterArr(self.formEleData, 'beginYear').opts = yearOpts.filter(function (y) { return y.val > self.reduceBeginYear && selectedYear.indexOf(y.val + 1) === -1 })
				...
				helperJs.filterArr(self.formEleData, 'endYear').opts = helperJs.filterArr(self.formEleData, 'beginYear').opts.filter(function (y, i, arr) {
			...
			ckAddAndTakeOkFn () {
				...
				helperJs.validateAllFn(0, helperJs.collectValidateVm(self, []), self, function () {
					...
					if (self.adjustData.purposeType === '自己填写') {
						self.adjustData.purpose && curAdjustData[curAdjustData.length - 1].push(self.adjustData.purpose)
					} else {
						curAdjustData[curAdjustData.length - 1].push(self.adjustData.purposeType)
					}
			...
			deleteAllAccount (type) {
				let self = this
				if (type == 'add') {
					helperJs.filterArr(self.curInsData.reduceBaofData.addAndTakeData, type).adjustData = []
					if (helperJs.filterArr(self.curInsData.reduceBaofData.addAndTakeData, 'take')) {
						helperJs.filterArr(self.curInsData.reduceBaofData.addAndTakeData, 'take').adjustData = []
					}
				} else if (type == 'take') {
					helperJs.filterArr(self.curInsData.reduceBaofData.addAndTakeData, type).adjustData = []
				}
			...
			ckPopupOkFn () {
				this.$set(this.curModuleData.curInsData, 'reduceBaofData', this.curInsData.reduceBaofData)
				this.ckClosePopup()
			},
			ckClosePopup () {
