		...
		<table>
			<caption position="top" class="clearFloat">
				...
				<div class="addAndTakeBtn">
					<span @click="gotoAdjustBaof">减保领取 <i class="iconfont icon-more_circle"></i></span>
				</div>
			</caption>
			<tbody v-if="insData.reduceBaofData && insData.reduceBaofData.addAndTakeData" @click="gotoAdjustBaof">
				<tr v-for="account in insData.reduceBaofData.addAndTakeData" v-if="account.adjustData && account.adjustData.length">
					<td style="text-align: center;">领取</td>
					<td class="clearFloat">
...
export default {
	...
	methods: {
		gotoAdjustBaof () {
			...
			self.allInputData.popupData.popupName = 'PopupReduceBaof'
		}
	}
}
</script>
