...
(function(root, factory) {
	...
	**/
	Mobilebone.ajax = function(aOrFormOrObj) {
        ...
        //     ele_mask.innerHTML = '<div class="loading-symbol"></div>';
        //     if (typeof attr_mask == "string") {
        ...
        // TODO 使用自己的popLayer API控制遮罩和Loading
        if (ele_mask == null && typeof attr_mask == "string") {
            ele_mask = document.createElement("div");
            ele_mask.className = this.classMask;
            ele_mask.innerHTML = '<div class="loading-symbol"></div>';
            aOrFormOrObj.appendChild(ele_mask);
		...
		xhr.onload = function() {
			...
			// hide loading
            // TODO 使用自己的遮罩API，暂时未估计inline loading symbol的场景
            $.popLayer('remove');
		...
		// 识别为XHR，如果今后因为跨域需要避免这么做，服务器可以使用上面的Type和From来判断。
	...
	Mobilebone.init = function() {	
		...
		if (hash == "" || hash == "#") {
			...
			this.ajax({
				...
				dataType: "unknown",
				container: container,
				// TODO 由Util接管初始化时的ajax error
				error: window.Util ? Util.handlePageAjaxError : function() {
