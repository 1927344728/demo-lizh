...
let appShareConfig = null;
let h5ButtonShareCfg = null;
...
function initShareAction(
  ...
  if (shareType && shareType.length) {
    shareType.map(action => {
      const sConfig =
        shareItemInfo &&
        shareItemInfo.filter(item => item.action === action)[0];
      // 小程序分享
 ...
 * @return   {[type]}             [description]
 */
function nativeShare(pageGroup) {
  // 原生分享功能，功能集合
 ...
 * @return   {[type]}               [description]
 */
function h5ButtonShare(pageGroup) {
  if (appBridge && appBridge.isApp()) {
 ...
 * @return   {[type]}                   [description]
 */
function wechatShare({ title, link, imgUrl, desc, success }, ticketLink) {
